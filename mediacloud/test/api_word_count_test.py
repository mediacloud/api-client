...
class ApiWordCountTest(ApiBaseTest):
    ...
    def testSort(self):
        term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK)
        last_count = None
    ...
    def testNumWords(self):
        ...
        term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK, num_words=101)
        self.assertEqual(len(term_freq), 101)

    def testStopWords(self):
        term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK)
        term_freq_with_stopwords = self._mc.wordCount('puppy', QUERY_LAST_WEEK, include_stopwords=True)
        self.assertNotEqual(term_freq[0]['term'], term_freq_with_stopwords[0]['term'])

    def testStats(self):
        term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK, include_stats=True)
        self.assertTrue('stats' in term_freq.keys())
        self.assertTrue('words' in term_freq.keys())

    def testBigram(self):
        term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK, ngram_size=2)
        for term in term_freq:
            self.assertEqual(len(term['term'].split(' ')), 2)

    def testRandomSeed(self):
        term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK)
        random_term_freq = self._mc.wordCount('puppy', QUERY_LAST_WEEK, random_seed=20)
